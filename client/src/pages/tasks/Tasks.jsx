...
const TaskCard = ({ task, onStatusChange, onDelete, onView, canManage }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-gray-800 rounded-xl p-3 sm:p-4 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow cursor-pointer"
      onClick={() => onView(task)}
    >
      {/* Priority + Delete */}
    ...
    <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
            {["kanban", "list"].map((v) => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={`px-2.5 sm:px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-all ${
                  view === v
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-3">
        {[
          ...
          <div
            key={`stat-${s.label}`}
            className={`${s.color} rounded-xl p-2.5 sm:p-3 text-center`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {COLUMNS.map((col) => (
            <div key={col.id} className="space-y-3">
              ...
              <div
                className={`min-h-[150px] sm:min-h-[200px] ${col.bg} rounded-xl p-2 sm:p-3 space-y-2 sm:space-y-3 border-t-2 ${col.color}`}
              >
                <AnimatePresence>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded font-medium flex-shrink-0 ${p.bg} ${p.color}`}
                      >
                        {task.priority}
                      </span>
                    </div>
              ...
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700/50">
                  {[
                    ...
                    <th
                      key={`th-${h}`}
                      className="text-left px-4 lg:px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {list.map((task, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`px-4 lg:px-6 py-3 lg:py-4 text-sm whitespace-nowrap ${
                          isOverdue
